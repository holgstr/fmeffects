...
FMEs make use of this instinct and apply it straightforwardly to a ML model.\
...
This is just the extension of the univariate FME to two features $x_{j}, x_{k}$ which are affected simultaneously. Therefore, the step size becomes a vector $\mathbf{h} = (h_{j}, h_{k})$, where $h_{j}$ denotes the change in $x_{j}$ and $h_{k}$ the change in $x_{k}$:\
\
...
Equivalent to the step size $h_{j}$ of a numerical feature, we select the category of interest $c_{h}$ for a categorical feature $x_{j}$. In theory, we could compute exactly as many FMEs for a categorical feature as it has categories. For a specific category, the FME is:\
\
...
Marginal effects (ME) are already a widely used concept to interpret statistical models. **However, we believe they are ill-suited to interpret feature effects in most state-of-the-art ML models. Here, we explain why you should abandon MEs in favor of FMEs:** \
...
- it is conceptually simple and researchers can discuss the interpretations because they understand how they are computed
...
[1]: https://holgstr.github.io/fme/articles/fme.html#categorical-features
