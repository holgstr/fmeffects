...
FMEs are a model-agnostic interpretation method, i.e., they can be applied to any regression and binary classification model. The `fme` package supports model from the `caret` and `mlr3` libraries. Let's try it with a random forest using the `ranger` algorithm:
