...
The `fme` package computes, aggregates and visualizes forward marginal effects (FMEs) for any supervised machine learning model. For a more in-depth understanding, refer to the [paper][1]. The package has three main functions: `fme()` computes FMEs for a given model and data, and `came()` finds subspaces of the feature space where FMEs are more homogeneous. Furthermore, `ame()` provides an informative overview of the entire prediction function w.r.t. all features by using average marginal effects (AMEs) based on FMEs.
...
FMEs are a model-agnostic interpretation method, i.e., they can be applied to any regression or (binary) classification model. Before you can compute FMEs, you need a trained model. The `fme` package supports models from the `caret` and `mlr3` libraries. Let's try it with a random forest using the `ranger` algorithm:
...
Here, the AME of `rain` is -55. Therefore, while holding all other features constant, rainy weather can be expected to reduce bike sharing usage by 55. \
...
In case we just want to produce an informative overview of all feature effects, we can use the `ame()` function:
